...
const setup = () => {
  const send = jest.fn();
  // const WebSocketMock = createWebSocketMock({onSend: send});
  // const physicalChannel = new WebSocketChannel({newSocket: () => new WebSocketMock('ws://localhost:1234')});
  // const physicalTextChannel = new TextChannel(physicalChannel);
  // const codec = new JsonCompactMsgCodec();
  // const logicalChannel = new MsgCodecLogicalChannel<string>({channel: physicalTextChannel, codec});
  // const compactLogicalChannel = new CompactToNativeTransform(logicalChannel) as LogicalChannel<CompactServerMessage[], CompactClientMessage[]>;
  // const bufferedChannel = new BufferedLogicalChannel({channel: compactLogicalChannel, bufferTime: 1});
